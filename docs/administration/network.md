...
Keep local console access available before changing the connection you use to
manage the appliance. Sign in as an administrator, open **System → Settings →
Network**, select an interface and choose **Configure**. Review DHCP or static
IPv4 settings; for Wi-Fi, select or enter the network name and credentials.
...
The dashboard provides **System → Settings → Network** for administrators.
