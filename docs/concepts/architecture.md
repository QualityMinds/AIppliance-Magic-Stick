...
[Private Mesh](../user-guide/private-mesh.md) extends the existing LiteLLM/model-catalog path
through an optional core module, not a second inference stack. It adds one supervised
transport/control-plane Pod with persistent identity, scoped dynamic aliases and
a loopback export bridge. All appliance inference still passes through LiteLLM.
The creator's signed membership is independent of Iroh relay connectivity.
