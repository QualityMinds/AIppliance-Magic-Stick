...
own bounded preparation profile. See [host package profiles](../administration/host-management.md).
...
- `shared-gtt`: GTT is larger than VRAM and KFD agrees with TTM. Capacity is
  ...
  host RAM budgets after safety headroom. The resulting Linux-capped capacity
  can be smaller than the firmware carve-out. Consumers preserve the host's
  corroborated domain instead of comparing that capped number with fixed VRAM
  and incorrectly treating it as contradictory evidence.
...
contract](../administration/host-management.md#fixed-and-dynamic-gpu-memory).
...
See [host management](../administration/host-management.md), including the bounded experiment mode
...
[GPU operations](../administration/troubleshooting/overview.md), [modules](module-catalog.md), and
[operator orchestration](../concepts/controllers.md).
