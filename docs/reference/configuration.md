...
Post-install Ethernet/Wi-Fi configuration is available in **System → Settings → Network**.
It uses runtime host operations, not deployment metadata or public Git values.
See [network management](../administration/network.md) for confirmation and recovery.
...
The shared [post-install host workflow](../administration/host-management.md) uses immutable
...
inputs. See [model-catalog.md](../concepts/model-routing.md).
