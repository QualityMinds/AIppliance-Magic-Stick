...
describe('host shared GPU memory', () => {
  ...
  it('allows reviewing an over-limit correction without another slider movement', async () => {
    host.gpuMemory!.systemMemoryMi = 62901;
    host.gpuMemory!.currentDynamicLimitMi = 100 * 1024;
    mount();
    expect(dynamicSlider()).toHaveValue(String(45 * 1024));
    expect(screen.getByText('Draft limited to 45 GiB')).toBeInTheDocument();
    expect(review()).toBeEnabled();
    expect(writes).toHaveLength(0);
    await userEvent.click(review());
    const dialog = screen.getByRole('dialog');
    expect(dialog).toHaveTextContent('100 GiB → 45 GiB');
    const confirm = within(dialog).getByRole('button', {name: 'Apply memory configuration'});
    expect(confirm).toBeDisabled();
    await userEvent.type(within(dialog).getByLabelText('Type example-node to confirm'), 'example-node');
    await userEvent.click(confirm);
    await waitFor(() => expect(writes).toHaveLength(1));
    expect(writes[0]).toMatchObject({gpuMemory: {carveoutIndex: 4, dynamicLimitMi: 45 * 1024}});
  });

  it('does not turn safe rounding near the maximum into a corrective change', () => {
    host.gpuMemory!.systemMemoryMi = 62901;
    host.gpuMemory!.currentDynamicLimitMi = 46200;
    mount();
    expect(dynamicSlider()).toHaveValue(String(45 * 1024));
    expect(screen.getByText('Draft rounded to 45 GiB')).toBeInTheDocument();
    expect(review()).toBeDisabled();
    expect(writes).toHaveLength(0);
  });
