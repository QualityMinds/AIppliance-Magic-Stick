  ...
  return <section className="host-gpu-memory stack compact" aria-label={`Shared GPU memory on ${host.name}`}>
    ...
    {canAdminister(session) && memory.supported && complete && <>
      <div className="estimate stack compact">
        ...
        {!dirty && draft.dynamicLimitMi !== memory.currentDynamicLimitMi && <div className="inline-info"><span className="muted">Draft rounded to {formatMi(draft.dynamicLimitMi)}</span><InfoPopover label="Rounded memory draft"><p className="memory-info-note">The current kernel limit does not match a selectable step. Move a slider to choose a change. Nothing is applied automatically.</p></InfoPopover></div>}
