...
const MemoryControls = ({host, memory, session, stale, hideHeading}: {host: ManagedHost; memory: HostGpuMemory; session: Session; stale: boolean; hideHeading?: boolean}) => {
  ...
  const currentLimitExceedsAllowance = complete && memory.currentDynamicLimitMi! > projectedRam(memory.currentCarveoutIndex!) - reserve;
  const reviewable = dirty || currentLimitExceedsAllowance;
  ...
  const mutation = useMutation({
    mutationFn: () => {
      if (!pending || blocked || !canAdminister(session) || !reviewable || !valid || !changed) throw new Error('Host state changed. Review the current memory configuration again.');
  ...
  return <section className={`host-gpu-memory stack compact${hideHeading ? ' host-gpu-memory-embedded' : ''}`} aria-label={`Shared GPU memory on ${host.name}`}>
    ...
    <dl className="facts">
      ...
      <div><dt className="inline-info">CPU/OS safety allowance<InfoPopover label={`CPU/OS safety allowance on ${host.name}`}><p className="memory-info-note">The dynamic ceiling must leave at least {formatMi(reserve)} outside GPU dynamic allocations. CPU processes still compete for the shared pool; this does not guarantee available memory or reserve RAM for a model.</p></InfoPopover></dt><dd>{formatMi(reserve)}</dd></div>
    </dl>
    ...
    {canAdminister(session) && memory.supported && complete && <>
      <div className="estimate stack compact">
        <div className="field"><span className="inline-info">Fixed GPU reservation (firmware)<InfoPopover label="Fixed GPU reservation (firmware)"><p className="memory-info-note">Only settings reported by this computer's firmware are offered. Changes remain a local draft until confirmed.</p></InfoPopover></span>
        ...
        <div className="field"><span className="inline-info">Dynamic GPU memory limit<InfoPopover label="Dynamic GPU memory limit"><p className="memory-info-note">Upper bound for on-demand GPU use of shared RAM, not reserved RAM. Steps of {formatMi(step)}.</p></InfoPopover></span>
        ...
        {!dirty && draft.dynamicLimitMi !== memory.currentDynamicLimitMi && <div className="inline-info"><span className="muted">Draft {currentLimitExceedsAllowance ? 'limited' : 'rounded'} to {formatMi(draft.dynamicLimitMi)}</span><InfoPopover label="Adjusted memory draft"><p className="memory-info-note">{currentLimitExceedsAllowance
          ? `The active dynamic limit exceeds the maximum allowed with the current fixed reservation and ${formatMi(reserve)} CPU/OS allowance. Review this draft to apply the correction; the active limit stays unchanged until the confirmed operation completes.`
          : 'The current kernel limit does not match a selectable step. Move a slider to choose a change. Nothing is applied automatically.'}</p></InfoPopover></div>}
      ...
      <div className="form-actions"><Button variant="primary" disabled={controlsDisabled || !reviewable || !changed || !valid} onClick={() => {
