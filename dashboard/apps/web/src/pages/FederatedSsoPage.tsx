...
export const FederatedSsoPage = () => {
  ...
  return <div className="stack">
    ...
    <div className={`notice ${feature.available ? 'notice-good' : 'notice-warn'}`}>{feature.available ? 'Federated SSO entitlement active. Dashboard-managed federation is available.' : verificationUnavailable ? 'License verification is currently unavailable. External login is blocked fail closed; viewing and recovery deletion remain available.' : feature.licensed ? 'The entitlement is present, but the Federated SSO implementation is unavailable.' : 'A valid Free Registered or Commercial Federated SSO entitlement is required to add or change providers.'}</div>
    ...
    <Panel title="Recovery and enforcement"><p className="muted">Local login and recovery stay available. Invalid or expired entitlements block external broker login and disable external providers without deleting their configuration or local users. Restoring a license does not automatically re-enable providers. Deletion remains available without a license for recovery.</p></Panel>
