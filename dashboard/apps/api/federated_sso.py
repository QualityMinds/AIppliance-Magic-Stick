...
"""Fail-closed integration for registered or commercial federated SSO."""
...
def plugin():
    try:
        return importlib.import_module("magicstick_core.federated_sso")
...
class FederatedSso:
    ...
    def save(self, principal, payload, *, alias=None, request_id=""):
        ...
        with LOCK:
            ...
            try:
                ...
                # Metadata/network work can outlive the entitlement checked at entry.
    ...
    def enforce_entitlement(self):
        """Disable every external broker, preserving its config and local login."""
        ...
        with LOCK:
            for provider in self._providers():
                # Social/custom brokers use other provider IDs (e.g. google),
                # but are still external identity-provider instances. Local
                # users, authentication flows and OIDC clients are separate.
