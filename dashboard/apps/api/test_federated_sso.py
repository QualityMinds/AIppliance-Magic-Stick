...
from magicstick_core import federated_sso as policy  # noqa: E402
...
class FederatedSsoLifecycleTests(unittest.TestCase):
    ...
    def test_expiry_disables_all_external_brokers_without_touching_local_login(self):
        protocols = ("oidc", "saml", "google", "microsoft", "custom-broker")
        for protocol in protocols:
            self.keycloak.providers[protocol] = {
                "alias": protocol, "providerId": protocol, "enabled": True,
                "config": {"clientSecret": "retained-provider-secret"},
            }
        before = copy.deepcopy(self.keycloak.providers)
        self.license.available = False
        self.assertEqual(self.service.enforce_entitlement(), len(protocols))
        for alias, provider in before.items():
            self.assertEqual(self.keycloak.providers[alias], {**provider, "enabled": False})
        writes = [call for call in self.keycloak.calls if call[0] != "GET"]
        self.assertTrue(all("/identity-provider/instances/" in path for _, path, _ in writes))
        self.assertFalse(any("/users" in path or "/authentication" in path for _, path, _ in writes))
        self.assertEqual(self.service.enforce_entitlement(), 0)

    def test_valid_entitlement_preserves_external_brokers(self):
        self.keycloak.providers["social"] = {"alias": "social", "providerId": "google", "enabled": True}
        before = copy.deepcopy(self.keycloak.providers)
        self.assertEqual(self.service.enforce_entitlement(), 0)
        self.assertEqual(self.keycloak.providers, before)
        self.assertFalse(self.keycloak.calls)

    def test_expiry_during_existing_provider_update_keeps_configuration_disabled(self):
        created = self.service.save(self.principal, oidc_payload())
        with patch.object(self.license, "require_capability", side_effect=[None,
                          LicenseError("unlicensed", "Entitlement expired.", 403)]):
            with self.assertRaises(LicenseError):
                self.service.save(self.principal, oidc_payload(
                    expectedRevision=created["providers"][0]["revision"]), alias="company")
        self.assertIn("company", self.keycloak.providers)
        self.assertFalse(self.keycloak.providers["company"]["enabled"])
        self.assertTrue(self.keycloak.mappers["company"])
