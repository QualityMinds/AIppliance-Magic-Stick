...
class LicenseTests(unittest.TestCase):
    def setUp(self):
        ...
        self.claims = {'version': 1, 'edition': 'commercial', 'product': 'magicstick', 'issuer': 'magicstick', 'licenseId': 'test-license',
    ...
    def test_commercial_production_requires_a_matching_entitlement(self):
        with patch('licensing.installed_capabilities', return_value={'commercial-production'}):
            free = self.service.status()
            production = next(feature for feature in free['features'] if feature['id'] == 'commercial-production')
            self.assertTrue(production['implemented'])
            self.assertFalse(production['licensed'])
            self.assertFalse(production['available'])

            activated = self.service.activate(self.document(), free['revision'])
            production = next(feature for feature in activated['features'] if feature['id'] == 'commercial-production')
            self.assertTrue(production['licensed'])
            self.assertTrue(production['implemented'])
            self.assertTrue(production['available'])
            self.assertTrue(all(not feature['available'] for feature in activated['features']
                                if feature['id'] != 'commercial-production'))

    def test_federated_sso_requires_its_matching_entitlement(self):
        claims = dict(self.claims)
        claims['features'] = ['federated-sso']
        claims['edition'] = 'free-registered'
        with patch('licensing.installed_capabilities', return_value={'commercial-production', 'federated-sso'}):
            free = self.service.status()
            federation = next(feature for feature in free['features'] if feature['id'] == 'federated-sso')
            self.assertTrue(federation['implemented'])
            self.assertFalse(federation['available'])

            activated = self.service.activate(self.document(claims), free['revision'])
    ...
    def test_request_download_uses_selected_modules_ttl_and_installation_without_activation(self):
        status = self.service.activate(self.document(), self.service.status()['revision'])
        before = copy.deepcopy(self.store.secret)
        with patch('licensing.time.time', return_value=self.now):
            result = self.service.request_document({'customer': ' Example organization ', 'edition': 'commercial',
                'features': ['commercial-production', 'federated-sso'], 'ttlSeconds': 48 * 3600})
        claims = validate_claims(strict_json(result['content']))
        self.assertEqual(claims['customer'], 'Example organization')
        self.assertEqual(claims['features'], ['commercial-production', 'federated-sso'])
        self.assertEqual(claims['issuedAt'], self.now)
        self.assertEqual(claims['notBefore'], self.now)
        self.assertEqual(claims['expiresAt'], self.now + 48 * 3600)
        self.assertEqual(claims['installationId'], status['installationId'])
        self.assertEqual(result['filename'], f"magicstick-license-request-{claims['licenseId']}.json")
        self.assertEqual(str(uuid.UUID(claims['licenseId'])), claims['licenseId'])
        with self.assertRaises(LicenseError):
            self.service.activate(result['content'], status['revision'])
        self.assertEqual(before, self.store.secret)

    def test_bad_request_does_not_initialize_or_change_license_state(self):
        valid = {'customer': 'Example organization', 'edition': 'free-registered', 'features': ['federated-sso'], 'ttlSeconds': 3600}
        invalid = [None, [], {}, {**valid, 'installationId': str(uuid.uuid4())},
                   {**valid, 'privateKey': 'not-accepted'}]
        invalid += [{**valid, 'ttlSeconds': value} for value in (None, True, False, 0, -1, 1.5, '3600', 253402300800)]
        invalid += [{**valid, 'features': value} for value in (None, [], 'private-mesh', ['unknown'], ['private-mesh', 'private-mesh'], [True], [{}])]
        invalid += [{**valid, 'customer': value} for value in (None, False, '', '   ', 'x' * 161, 'a\nb')]
        for payload in invalid:
            with self.subTest(payload=payload), self.assertRaises(LicenseError):
                self.service.request_document(payload)
            self.assertIsNone(self.store.secret)

    def test_request_can_be_generated_without_trusted_keys_and_signed_by_existing_issuer(self):
        self.trust.unlink()
        result = self.service.request_document({'customer': 'Example organization', 'edition': 'free-registered',
            'features': ['federated-sso'], 'ttlSeconds': 30 * 86400})
        again = self.service.request_document({'customer': 'Example organization', 'edition': 'free-registered',
            'features': ['federated-sso'], 'ttlSeconds': 30 * 86400})
        self.assertNotEqual(result['filename'], again['filename'])
        claims = strict_json(result['content'])
        self.assertEqual(self.service.status()['state'], 'missing')
        claims_path = self.directory / result['filename']
        claims_path.write_text(result['content'], encoding='utf-8')
        private_key = self.directory / 'ephemeral-signing-key.pem'
        private_key.write_bytes(self.key.private_bytes(serialization.Encoding.PEM,
            serialization.PrivateFormat.PKCS8, serialization.NoEncryption()))
        output = self.directory / 'signed-license.json'
        with redirect_stdout(io.StringIO()), redirect_stderr(io.StringIO()):
            license_issuer.main(['issue', '--claims', str(claims_path), '--private-key', str(private_key),
                '--kid', 'test', '--output', str(output)])
        verified = verify_document(output.read_text(), public_keys({'keys': {'test': self.pem}}),
            claims['installationId'], now=claims['notBefore'])
    ...
    def test_capability_checks_never_override_authorization_or_implementation(self):
        ...
        self.service.activate(self.document(), self.service.status()['revision'])
        with patch('licensing.installed_capabilities', return_value=set()), self.assertRaises(LicenseError) as caught:
            self.service.require_capability('federated-sso', authorized=True)
        self.assertEqual(caught.exception.code, 'not_implemented')

    def test_edition_is_signed_and_entitlements_cannot_escalate_free_registration(self):
        self.assertEqual(set(FEATURES), {'commercial-production', 'federated-sso'})
        self.assertEqual(self.service.status()['edition'], 'free')
        for edition, features in [('free-registered', ['federated-sso']), ('commercial', list(FEATURES))]:
            claims = {**self.claims, 'edition': edition, 'features': features}
            status = self.service.activate(self.document(claims), self.service.status()['revision'])
            self.assertEqual(status['edition'], edition)
            self.assertEqual(status['claims']['features'], features)
        for changes in ({'edition': 'free'}, {'edition': []},
                        {'edition': 'free-registered'}, {'features': ['federated-sso']},
                        {'features': ['resource-sharing']}, {'features': ['private-mesh']}):
            with self.subTest(changes=changes), self.assertRaises(LicenseError):
                validate_claims({**self.claims, **changes})
