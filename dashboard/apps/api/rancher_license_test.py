...
'''
    ...
    try:
        ...
        for method, path, body in [('GET', '/api/license', None), ('GET', '/api/license/export', None), ('POST', '/api/license/request', {'customer': 'Example organization', 'edition': 'free-registered', 'features': ['federated-sso'], 'ttlSeconds': 3600}), ('POST', '/api/license/validate', {'document': '{}'}), ('PUT', '/api/license', {'document': '{}', 'expectedRevision': '0'})]:
            request(method, path, body, role='viewer', expected=403)
        status = request()
        assert status['state'] == 'missing'
        assert status['edition'] == 'free'
        assert set(feature['id'] for feature in status['features']) == {'federated-sso', 'commercial-production'}
        assert all(feature['implemented'] and not feature['licensed'] and not feature['available']
        ...
        draft = {'customer': 'Example organization', 'edition': 'commercial', 'features': list(FEATURES), 'ttlSeconds': 3600}
        request('POST', '/api/license/request', draft, csrf=False, expected=403)
        request('POST', '/api/license/request', {**draft, 'ttlSeconds': 0}, expected=400)
        download = request('POST', '/api/license/request', draft)
        claims = json.loads(download['content'])
        assert claims['installationId'] == status['installationId'] and claims['features'] == list(FEATURES)
        assert claims['expiresAt'] - claims['notBefore'] == 3600
        assert download['filename'].endswith('.json') and request()['revision'] == status['revision']
        assert not request('POST', '/api/license/validate', {'document': download['content']})['candidate']['valid']
        ...
        assert result['valid'] and result['edition'] == 'commercial'
        assert all(feature['licensed'] and feature['implemented'] and feature['available']
