...
source = importlib.util.spec_from_file_location("sharing_test", ROOT / "core/magicstick_core/sharing.py")
sharing_core = importlib.util.module_from_spec(source)
source.loader.exec_module(sharing_core)
...
class SharingTests(unittest.TestCase):
    def setUp(self):
        self.modules = patch.dict(sys.modules, {
            "magicstick_core": types.SimpleNamespace(),
            "magicstick_core.sharing": sharing_core,
        })
        self.modules.start()
        ...
        self.service = InstanceAccess(user=lambda name: copy.deepcopy(self.users[name]),
    ...
    def test_sharing_is_available_without_license_service_but_acl_remains_enforced(self):
        with patch("licensing.LicenseService.status", side_effect=AssertionError("Sharing must not read licensing")):
    ...
    def test_missing_policy_code_denies_only_private_and_license_failure_is_irrelevant(self):
        with patch.dict(sys.modules, {"magicstick_core": None, "magicstick_core.sharing": None}):
            self.assertFalse(self.service.can_use(self.private, self.principal("alice")))
            self.assertTrue(self.service.can_use(self.public, self.principal("alice")))
        with patch("licensing.LicenseService.status", side_effect=OSError("Unavailable")):
            self.assertTrue(self.service.can_use(self.private, self.principal("alice")))
    ...
    def test_ids_not_display_names_and_strict_policy_validation(self):
        for policy in ({"mode": "selected", "users": ["alice", "alice"]}, {"mode": "selected", "groups": "team"},
            ...
            with self.subTest(policy=policy), self.assertRaises(ValueError):
                sharing_core.validate_policy(policy)
