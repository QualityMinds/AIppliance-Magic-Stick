...
(async () => {
  ...
  try {
    ...
    const feature = {id: 'federated-sso', name: 'Federated SSO', available: false, implemented: true, licensed: false, reason: 'unlicensed'};
    ...
    await page.route('**/api/**', async route => {
      ...
      if (url.pathname === '/api/license') data = {edition: 'free', state: 'missing', valid: false, message: 'Free mode.', installationId: 'example-installation', revision: '1', checkedAt: 1, hasDocument: false, trustedKeyIds: [], features: [feature]};
      ...
      if (url.pathname.endsWith('/access')) {
        ...
        data = {name: 'hermes-example', revision: '4', sharing: {mode: 'all', users: [], groups: []}, authentication: 'sso', guardReady: true};
      }
    ...
    for (const [title, filename, source] of [
      ...
      ['Business Source License 1.1', 'MagicStick-BSL.txt', '../../../LICENSE'],
      ['MIT Change License · after three years per version', 'MIT-CHANGE.txt', '../../../licenses/MIT-CHANGE.txt'],
      ['Third-party notices', 'THIRD_PARTY_NOTICES.md', '../../../THIRD_PARTY_NOTICES.md'],
      ['JavaScript dependency license texts', 'npm-notices.txt', '../../../licenses/third-party/npm.txt'],
      ['Python dependency license texts', 'python-notices.txt', '../../../licenses/third-party/python.txt'],
    ]) {
