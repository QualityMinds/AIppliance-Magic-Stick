"""Offline edition and entitlement verification for Magic Stick.
...
MAX_LICENSE_TIMESTAMP = 253402300799
SECRET_NAME = "magicstick-license"
FEATURES = {
    "commercial-production": "Commercial production use",
...
def installed_capabilities():
    try:
        importlib.import_module("magicstick_core.federated_sso")
        return set(FEATURES)
...
def validate_claims(claims):
    required = {"version", "product", "issuer", "licenseId", "customer", "issuedAt", "notBefore", "expiresAt", "features", "edition"}
    ...
    if any(type(v) is not int or v < 0 or v > MAX_LICENSE_TIMESTAMP for v in dates) or not dates[0] <= dates[1] < dates[2]:
    ...
    if not isinstance(edition, str) or edition not in {"free-registered", "commercial"}:
        raise LicenseError("invalid_edition", "Select Free Registered or Commercial.")
    expected = {"federated-sso"} if edition == "free-registered" else set(FEATURES)
    if set(features) != expected:
        raise LicenseError("invalid_entitlements", "Entitlements do not match the selected edition.")
...
def verify_document(document, keys, installation_id, now=None):
    ...
    if claims.get("installationId") and claims["installationId"] != installation_id:
        ...
        state, message = "expired", "License has expired. Core functions and existing access policies remain available; Federated SSO is disabled."
...
class LicenseService:
    ...
    def status(self, state=None):
        ...
        result = self._result(data["license.json"], installation_id) if data.get("license.json") else {
            "state": "missing", "message": "Free mode. No license file required for eligible use under the Additional Use Grant. Federated SSO requires registration.", "valid": False,
        }
        ...
        try:
            ...
            trusted = []
        return {**result, "edition": result["claims"]["edition"] if result["valid"] else "free",
                "installationId": installation_id, "revision": secret["metadata"]["resourceVersion"],
    ...
    def request_document(self, payload):
        """Build unsigned issuer input without changing installed entitlements."""
        if not isinstance(payload, dict) or set(payload) != {"customer", "features", "ttlSeconds", "edition"}:
            raise LicenseError("invalid_request", "Provide a customer reference, edition, entitlements and TTL only.")
        now = int(time.time())
        ttl = payload["ttlSeconds"]
        if type(ttl) is not int or ttl <= 0 or ttl > MAX_LICENSE_TIMESTAMP - now:
            raise LicenseError("invalid_ttl", "TTL must be a positive whole number of seconds within the supported validity range.")
        if not isinstance(payload["features"], list) or not payload["features"]:
            raise LicenseError("invalid_request", "Select the edition's entitlements.")
        customer = payload["customer"]
        claims = validate_claims({
            "version": 1, "product": "magicstick", "issuer": "magicstick",
            "licenseId": str(uuid.uuid4()), "customer": customer.strip() if isinstance(customer, str) else customer,
            "issuedAt": now, "notBefore": now, "expiresAt": now + ttl,
            "features": payload["features"], "edition": payload["edition"],
        })
        # Validate all user input before even initializing installation state.
        claims["features"] = [feature for feature in FEATURES if feature in claims["features"]]
        claims["installationId"] = self.state()[2]
        return {"filename": f"magicstick-license-request-{claims['licenseId']}.json",
                "content": json.dumps(claims, indent=2, ensure_ascii=False) + "\n"}
    ...
    def require_capability(self, feature, *, authorized):
        ...
        if not item or not item["licensed"]:
            raise LicenseError("unlicensed", "A valid Free Registered or Commercial license with the required entitlement is needed.", 403)
        if not item["implemented"] or not item["available"]:
            raise LicenseError("not_implemented", "This capability is not implemented.", 409)
