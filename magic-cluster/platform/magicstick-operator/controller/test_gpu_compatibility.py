...
class GpuCompatibilityTests(unittest.TestCase):
    ...
    def test_dra_recovery_is_not_reported_as_missing_host_driver(self):
        compatibility = self.controller["gpu_compatibility_status"]([self.node], self.features, self.catalog, self.activation)
        self.controller["GPU_SHARING_STATE"] = {"mode": "dra-shared", "phase": "Blocked",
            "message": "AMD host driver is ready; GPU allocation is blocked by a legacy claim."}
        result = self.hardware_status(compatibility)
        self.assertEqual(result["phase"], "Degraded")
        self.assertEqual(result["allocatableResources"], 0)
        self.assertIn("host driver is ready", result["message"])
        self.assertNotIn("being installed", result["message"])
    ...
    def test_linux_capped_shared_capacity_below_carveout_preserves_shared_domain(self):
        self.change_host(firmwareReservedMi=65536, physicalMemoryMi=61 * 1024,
                         gpuAccessibleMi=61 * 1024, gpuCapacityMi=61 * 1024,
                         gpuAllocationMode="shared-gtt", gpuCapacitySource="kfd-topology")
        runtime = {"computeTarget": "amd-gpu", "engine": "OLlama", "vramMi": 50000, "memoryMi": 0,
                   "baseResourceProfile": "magicstick-amd-gpu:1"}
        with patch.dict(self.controller, {"compute_target_nodes": lambda *_: [self.node]}):
            self.controller["unified_memory_runtime"]({"metadata": {}, "spec": {}}, runtime, {})
        ...
        self.assertEqual(self.controller["gpu_memory_allocation"](self.host), ("shared-gtt", 61 * 1024))
        self.change_host(gpuCapacityMi=100 * 1024)
        self.assertEqual(self.controller["gpu_memory_allocation"](self.host), ("unknown", None))
