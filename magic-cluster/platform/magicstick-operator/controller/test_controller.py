...
def load_controller(*, cache_gate=False):
    ...
    if not cache_gate:
        namespace["model_cache_cleanup_active"] = lambda: False
...
class HelmAppInstanceTests(unittest.TestCase):
    ...
    def test_sharing_reconciliation_has_no_license_probe(self):
        controller = load_controller()
        resource_path = controller["resource_path"]
        self.assertNotIn("instance_sharing_entitled", controller)
        self.assertIn("sync_app_instance_access", controller)
        self.assertEqual(
    ...
    def test_model_contract_exposes_all_local_engines_with_explicit_target_matrix(self):
        crd = yaml.safe_load(
...
class HardwareOperatorManifestTests(unittest.TestCase):
    ...
    def test_vendor_charts_are_pinned_and_do_not_duplicate_nfd(self):
        ...
        self.assertEqual(str(nvidia["spec"]["chart"]["spec"]["version"]), "v26.7.0")
        self.assertFalse(nvidia["spec"]["values"]["nfd"]["enabled"])
        self.assertEqual(nvidia["spec"]["values"]["driver"]["version"], "595.91.07")
        self.assertEqual(nvidia["spec"]["values"]["driver"]["kernelModuleType"], "auto")
        self.assertTrue(nvidia["spec"]["values"]["clusterPolicy"]["deployCR"])
        self.assertFalse(nvidia["spec"]["values"]["gpuCluster"]["deployCR"])
        self.assertFalse(nvidia["spec"]["values"]["cdi"]["nriPluginEnabled"])
        toolkit_env = {entry["name"]: entry["value"] for entry in nvidia["spec"]["values"]["toolkit"]["env"]}
        self.assertEqual(toolkit_env["CONTAINERD_CONFIG"], "/var/lib/rancher/k3s/agent/etc/containerd/config.toml")
        self.assertEqual(toolkit_env["CONTAINERD_SOCKET"], "/run/k3s/containerd/containerd.sock")
        self.assertEqual(toolkit_env["RUNTIME_CONFIG_SOURCE"], "file=/var/lib/rancher/k3s/agent/etc/containerd/config.toml")
        self.assertEqual(toolkit_env["RUNTIME_DROP_IN_CONFIG"], "/var/lib/rancher/k3s/agent/etc/containerd/config-v3.toml.d/99-nvidia.toml")
