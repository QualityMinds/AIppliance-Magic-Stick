...
Options:
  --deployment-name NAME       Deployment name used in Flux paths.
  --hostname NAME              Hostname written to CIDATA/meta-data.
  --git-owner OWNER            GitHub owner or organization.
  --git-repo REPO              Deployment repository name.
  --git-branch BRANCH          Deployment branch.
  --flux-cluster-path PATH     Flux bootstrap path. Defaults to deployments/NAME/infra-cluster/flux-bootstrap.
  --flux-bootstrap-mode MODE   github or readonly-public. Default: github.
...
If FLUX_BOOTSTRAP_MODE=github, the Flux/GitHub token is read from
FLUX_GITHUB_TOKEN or prompted without echo.
USAGE
}
