...
For documentation changes, install the separate build-only requirements and check
both the Markdown sources and the complete static output:

```bash
python3 -m venv .build/docs-venv
.build/docs-venv/bin/pip install -r requirements-docs.txt
.build/docs-venv/bin/python -m unittest tests.test_docs
.build/docs-venv/bin/python tools/docs.py build
git diff --check
```

See [documentation maintenance](docs/development/documentation.md) for authoring,
legacy links and Pages publication, and the
[release checklist](docs/development/release-checklist.md) for release checks.
