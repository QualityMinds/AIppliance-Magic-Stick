...
The host-management role installs the fixed-scope model-cache inventory/cleanup
helper used by **System → Model cache**. No cache is deleted automatically.
See [model cache management](../docs/model-cache.md) for the protected workload
checks and the worker/API/controller upgrade contract.
...
Node from 20 seconds after boot and every minute, without changing eligibility
labels. Failed publication retries after 10 seconds rather than waiting a full
timer period; boot ID, kernel and Node UID checks remain mandatory.
...
Configurable UMA capacity is excluded from the hardware identity so an approved
carve-out change does not look like a replaced GPU; the host worker verifies the
exact requested memory settings separately after each restart.
