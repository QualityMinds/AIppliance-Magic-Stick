...
class Worker:
    ...
    def reconcile_memory(self, operation):
        ...
        try:
            ...
            if self.report.get("os") != current["memoryOs"]:
                raise RuntimeError("The OS identity changed during memory maintenance.")
            if self.memory.get("waitingForCompanionDriver") is True:
                ...
                return
            # Full driver identity is only comparable after the companion has
            # rebound; PCI hardware and the timeout still gate the wait above.
            if self.report.get("hardwareFingerprint") != current["memoryFingerprint"]:
                raise RuntimeError("Hardware, driver or firmware identity changed during memory maintenance.")
    ...
    def reconcile(self, operation):
        ...
        if not current or current.get("operationUid") != meta.get("uid"):
            ...
            try:
                ...
                if spec["action"] == "clear-model-cache":
                    ...
                    before = self.cache.get("reclaimableBytes", 0)
                    self.cache = model_cache.clear(self.node, kube, spec)
                    remaining = self.cache.get("reclaimableBytes", before)
                    self.update(operation, "Succeeded", "Model cache cleared. Removed approximately %.1f GiB of cached model files." % (max(0, before - remaining) / 2**30))
                    return
                ...
                message = str(error) if spec.get("action") == "clear-model-cache" and isinstance(error, ValueError) else "The requested host operation failed. No automatic retry or reboot will occur. Inspect the host management and network recovery services before retrying."
                self.update(operation, "Failed", message)
            return
        ...
        if current["nodeUid"] != self.node["metadata"]["uid"]:
            ...
            return
        if current["action"] == "clear-model-cache":
            self.update(operation, "Interrupted", "Cache cleanup was interrupted and was not repeated. Refresh the inventory before another request.")
            return
