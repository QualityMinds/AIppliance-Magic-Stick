...
class HostEvidenceTests(unittest.TestCase):
    def test_publication_retries_transient_boot_failure_without_five_minute_gap(self):
    ...
    def test_large_gtt_stays_shared_when_linux_capacity_is_below_carveout(self):
        self.allocation_fixture(64, 100, 100)
        self.write("/proc/meminfo", f"MemTotal: {61 * 1024**2} kB\n")
        report = preflight.collect(self.root, live=False)["nodeAnnotation"]
        self.assertEqual(report["firmwareReservedMi"], 64 * 1024)
        self.assertEqual(report["gpuCapacityMi"], 61 * 1024)
        self.assertEqual(report["gpuAccessibleMi"], 61 * 1024)
        self.assertEqual(report["gpuAllocationMode"], "shared-gtt")

    def test_approved_uma_change_does_not_look_like_replaced_hardware(self):
        self.allocation_fixture(0.5, 100, 100)
        prefix = "/sys/bus/pci/devices/0000:01:00.0/"
        self.write(prefix + "uma/carveout_options", "0: Minimum (512 MB)\n7: High (64 GB)")
        self.write("/proc/meminfo", f"MemTotal: {125 * 1024**2} kB\n")
        before = preflight.collect(self.root, live=False)
        self.write(prefix + "mem_info_vram_total", str(64 * 1024**3))
        self.write(prefix + "uma/carveout", "7")
        self.write("/proc/meminfo", f"MemTotal: {61 * 1024**2} kB\n")
        after = preflight.collect(self.root, live=False)
        self.assertEqual(before["hardwareFingerprint"], after["hardwareFingerprint"])
        self.assertNotEqual(before["nodeAnnotation"]["firmwareReservedMi"], after["nodeAnnotation"]["firmwareReservedMi"])
        self.assertEqual(after["nodeAnnotation"]["displayDevices"][0]["memoryTotalMi"], 64 * 1024)
        self.write("/sys/module/amdgpu/version", "changed-driver")
        self.assertNotEqual(after["hardwareFingerprint"], preflight.collect(self.root, live=False)["hardwareFingerprint"])

    def test_discrete_gpu_capacity_remains_part_of_hardware_identity(self):
        self.strix_fixture()
        self.device(device="73bf", address="0000:02:00.0", minor=129)
        path = "/sys/bus/pci/devices/0000:02:00.0/mem_info_vram_total"
        self.write(path, str(16 * 1024**3))
        before = preflight.collect(self.root, live=False)["hardwareFingerprint"]
        self.write(path, str(32 * 1024**3))
