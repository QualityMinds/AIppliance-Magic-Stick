...
def collect(root=Path("/"), live=True):
    ...
    # UMA changes deliberately resize VRAM across a reboot. Capacity belongs to
    # the memory plan, not hardware identity; the worker verifies it separately.
    uma_devices = {device["pciAddress"] for device in devices if device["memoryTopology"] == "shared"}
    fingerprint_data = {"os": report["os"], "kernel": release, "driverVersion": report["driver"]["version"], "firmwarePackage": firmware_version,
                        "displayDevices": [{key: value for key, value in device.items()
                                            if key != "memoryTotalMi" or device["pciAddress"] not in uma_devices} for device in display_devices],
